...
After adding metrics, analyze the `expression` field and generate `lineage:` entries connecting source tables to the metric:

```yaml
lineage:
  - id: lin-fact_orders-revenue
    from: fact_orders
    to: revenue
```

Rules:
- Lineage uses **table-level** granularity (not column-level). Reference the table that contains the columns used in the expression.
- If the expression references multiple tables, add one lineage entry per source table.
- The metric `id` is used as the `to` value in lineage entries — do not use the metric `name`.
- Validate that the referenced table IDs actually exist in `tables:` before adding lineage.
